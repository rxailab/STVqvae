...
RUN_DIR="${RUN_DIR:-$PROJECT_ROOT/wm_runs/vae_wm_rl_v6_plainlong}"
DEFAULT_LOCAL_PYTHON="$PROJECT_ROOT/../miniforge3/envs/stvqvae/bin/python"
DEVICE="${DEVICE:-cuda}"
TRANS_EPOCHS="${TRANS_EPOCHS:-120}"
RL_UNROLL_STEPS="${RL_UNROLL_STEPS:-8}"
RL_TRAIN_STEPS="${RL_TRAIN_STEPS:-1000000}"
N_TRAIN_UNROLL="${N_TRAIN_UNROLL:-8}"
...
echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] Starting v6_plainlong"
echo "RUN_DIR=$RUN_DIR"
echo "PYTHON_BIN=$PYTHON_BIN"
echo "DEVICE=$DEVICE TRANS_EPOCHS=$TRANS_EPOCHS RL_UNROLL_STEPS=$RL_UNROLL_STEPS RL_TRAIN_STEPS=$RL_TRAIN_STEPS N_TRAIN_UNROLL=$N_TRAIN_UNROLL"
...
exec "$PYTHON_BIN" -u full_train_eval.py \
  ...
  --filter_size 9 \
  --epochs 20 \
  --trans_epochs "$TRANS_EPOCHS" \
  --batch_size 1024 \
  --eval_batch_size 128 \
  --n_preload 0 \
  --no_load \
  --n_train_unroll "$N_TRAIN_UNROLL" \
