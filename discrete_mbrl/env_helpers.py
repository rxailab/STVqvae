...
class PredictedModelWrapper(Wrapper):
    ...
    def step(self, action):
        ...
        return obs, reward.item(), done, done, {}
...
# =============================================================================
# Helper: decide whether to use a fixed seed or diverse random seeds
# =============================================================================

def _should_use_random_seeds(env_name, random_seeds):
    """
    Returns True when the environment should get a fresh random seed
    every episode (diverse layouts for generalisation).

    Triggered by:
      - The caller passing random_seeds=True
      - The env_name containing '-rand'
    """
    return random_seeds or '-rand' in env_name.lower()


def _strip_rand_suffix(env_name):
    """Remove the '-rand' marker from an env name (case-insensitive)."""
    # Find -rand case-insensitively
    lower = env_name.lower()
    idx = lower.find('-rand')
    if idx != -1:
        env_name = env_name[:idx] + env_name[idx + len('-rand'):]
    return env_name


# =============================================================================
# Main environment factory
# =============================================================================

def make_env(env_name, replay_buffer=None, buffer_lock=None, extra_info=None,
             monitor=False, max_steps=None, random_seeds=False):
    """
    ...
    ----------
    env_name : str
        Environment identifier (may include modifiers like '-rand').
    replay_buffer : h5py.File, optional
        HDF5 replay buffer for trajectory recording.
    buffer_lock : threading.Lock, optional
        Lock for thread-safe buffer writes.
    extra_info : list[str], optional
        Extra info-dict keys to record.
    monitor : bool
        Wrap with SB3 Monitor.
    max_steps : int, optional
        Override the environment's max_steps.
    random_seeds : bool
        When True, every episode gets a fresh random seed so the agent
        sees diverse layouts.  Equivalent to adding '-rand' to env_name.
    """

    # --- Determine seed strategy once, up front ---
    use_random = _should_use_random_seeds(env_name, random_seeds)
    env_name = _strip_rand_suffix(env_name)  # Clean the name for gym.make()

    # ------------------------------------------------------------------
    # MiniGrid: Simple Stochastic
    # ------------------------------------------------------------------
    if env_name.lower() == 'minigrid-simple-stochastic':
        ...
        # Seed wrapper
        if use_random:
            wrappers.append(lambda env: RandomReseedWrapper(env, n_repeat=1))
        else:
            wrappers.append(lambda env: ReseedWrapper(env, seeds=[41]))

        env = gym.make('MiniGrid-Empty-6x6-v0')
        env.unwrapped.max_steps = max_steps or 10000

    # ------------------------------------------------------------------
    # MiniGrid: Key Stochastic
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # MiniGrid: Crossing Stochastic (with compact/ucompact variants)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # MiniGrid: Door Key Stochastic (with compact/ucompact variants)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # MiniGrid: Generic (including LavaCrossing, MultiRoom, Empty, etc.)
    # ------------------------------------------------------------------
        ...
        # ── NEW: diverse-seed support for ALL generic MiniGrid envs ──
        # This is the key change for LavaCrossing, MultiRoom, etc.
        if use_random:
            wrappers.append(lambda env: RandomReseedWrapper(env, n_repeat=1))
