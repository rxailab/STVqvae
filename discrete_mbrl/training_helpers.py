...
def get_optimized_args(args):
    ...
    '''
    # Cap workers at system max, but never auto-increase from 0
    # (0 means the user explicitly disabled multiprocessing via -pl 0)
    if not windows_multiprocessing_disabled and args.n_preload > 0:
        args.n_preload = min(args.n_preload, min(8, os.cpu_count() or 1))
...
def get_args(parser=None, apply_optimizations=True):
    ...
    parser.add_argument('--ema-decay', type=float, default=0.99,
                        help='EMA decay for VQ-VAE codebook updates (lower = faster tracking)')
...
def make_argparser(parser=None):
    ...
    parser.add_argument('--model_dir', type=str, default='.',
                        help='Root directory for model saves/loads (default: current directory)')
...
def add_model_args(parser):
    ...
    # ---- VQVAE codebook regularisation ----
    parser.add_argument(
        '--entropy_penalty_coef', type=float, default=0.0,
        help='Coefficient for the soft-entropy collapse penalty (0 = disabled). '
             'Penalises the gap between current soft-assignment entropy and log(K). '
             'Gradient flows back to the encoder. Try 0.01-0.1.')
    parser.add_argument(
        '--code_dropout_rate', type=float, default=0.0,
        help='Fraction of latent positions zeroed before decoding during training '
             '(0 = disabled). Forces the decoder to rely on multiple codes. Try 0.1-0.25.')
    parser.add_argument(
        '--mae_mask_ratio', type=float, default=0.0,
        help='Fraction of patch positions masked for MAE-style reconstruction '
             '(0 = disabled). Encourages codes to carry semantic meaning. Try 0.25-0.75.')
    parser.add_argument(
        '--mae_patch_size', type=int, default=4,
        help='Pixel size of each patch for MAE masking grid (default 4).')
    parser.add_argument(
        '--mae_loss_coef', type=float, default=1.0,
        help='Weight applied to the MAE reconstruction loss term (default 1.0).')
    parser.add_argument(
        '--ctx_aux_coef', type=float, default=1.0,
        help='Weight for the codes-only auxiliary reconstruction loss used with '
             'local_ctx_vqvae to prevent the ctx_encoder bypass from collapsing '
             'the codebook (default 1.0). Increase to 2.0 if collapse persists.')
    parser.add_argument(
        '--ctx_channels', type=int, default=64,
        help='Number of channels in the local_ctx_vqvae context encoder (default 64).')
    parser.add_argument(
        '--ctx_cond_type', type=str, default='concat',
