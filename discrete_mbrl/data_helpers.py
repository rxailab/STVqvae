...
class NStepReplayDataset(Dataset):
    def __init__(self, env_name, n_steps, transform=None, preload=False,
        ...
        with h5py.File(self.replay_buffer_path, 'r') as buffer:
            print(f'HDF5 file opened in {time.time() - h5_start:.2f}s')
...
def prepare_dataloaders(env_name, batch_size=256, randomize=True, n_step=1,
    ...
    #print(f'  Subsets created in {time.time() - subset_start:.2f}s')
    ...
    #print(f'  Validation dataset created in {time.time() - valid_start:.2f}s')
    ...
    valid_loader = create_fast_loader(
        ...
        prefetch_factor=prefetch_factor)
    #print(f'  Data loaders created in {time.time() - loader_start:.2f}s')

    #print(f'  Total prepare_dataloaders took {time.time() - start_time:.2f}s')
