...
class VQVAETrainer(BaseRepresentationLearner):
    def __init__(
        ...
        recon_loss_clip: float = 0,
        grad_clip: float = 0,
        # ---- codebook utilisation penalty ----
        # Mild gradient penalty when soft codebook entropy < log(K).
        # Reasonable starting value: 0.01–0.1.
        entropy_penalty_coef: float = 0.0,
        # ---- random code dropout ----
        # Fraction of latent positions zeroed before decoding (training only).
        # Reasonable starting value: 0.1–0.25.
        code_dropout_rate: float = 0.0,
        # ---- MAE-style masked reconstruction ----
        # Fraction of patch-grid positions to mask.
        mae_mask_ratio: float = 0.0,
        # Patch size in pixels (spatial resolution of the mask grid).
        mae_patch_size: int = 4,
        # Weight applied to the MAE reconstruction loss term.
        mae_loss_coef: float = 1.0,
        # ---- codes-only auxiliary loss (local_ctx_vqvae bypass prevention) ----
        # When > 0, an auxiliary reconstruction pass without context conditioning
        # is run each step to force the VQ codes to carry independent information.
        # Without this, local_ctx_vqvae collapses because the decoder bypasses the
        # VQ bottleneck through the continuous ctx_encoder(x) path.
        # Recommended value: 0.5–2.0 (matches scale of main recon_loss).
        ctx_aux_coef: float = 1.0,
    ):
        super().__init__(model, batch_size, update_freq, log_freq)
        self.model = model
        self.recon_loss_clip = recon_loss_clip
        ...
        self.ctx_aux_coef = ctx_aux_coef
    ...
    def calculate_losses(self, batch_data, return_stats: bool = False):
        ...
        # -------------------------
        # Forward pass
        # Code dropout is only active during training.
        # -------------------------
        code_dropout = self.code_dropout_rate if self.model.training else 0.0
        try:
            obs_recon, quantizer_loss, perplexity, oh_encodings = self.model(
                obs, code_dropout_rate=code_dropout)
            loss_dict["quantizer_loss"] = quantizer_loss
        except Exception as e:
        ...
        # -------------------------
        # Codes-only auxiliary loss (local_ctx_vqvae bypass prevention)
        # The ctx_encoder(x) path gives the decoder a full continuous view of the
        # input, bypassing the VQ bottleneck. Without this loss the decoder learns
        # to reconstruct from h_ctx alone → codebook collapse.
        # This auxiliary pass forces the local encoder + codes to be independently
        # sufficient for reconstruction.
        # -------------------------
        if self.ctx_aux_coef > 0 and self.model.training \
                and hasattr(self.model, 'ctx_encoder') \
                and self.model.ctx_encoder is not None:
            x_hat_codes, _, _, _ = self.model(obs, use_context=False)
            if x_hat_codes.shape != obs.shape:
                x_hat_codes = F.interpolate(
                    x_hat_codes, size=obs.shape[-2:], mode='nearest')
            aux_sq = (obs - x_hat_codes).pow(2)
            aux_loss = aux_sq.reshape(aux_sq.shape[0], -1).sum(-1).mean()
            loss_dict["ctx_aux_loss"] = self.ctx_aux_coef * aux_loss

        # -------------------------
        # Entropy penalty (training only)
        # Encourages diverse codebook usage by penalising the gap between the
        # current soft-assignment entropy and the theoretical maximum log(K).
        # Uses _last_soft_entropy stored by VectorQuantizerEMA.forward(), which
        # is differentiable w.r.t. encoder outputs through the distance logits.
        # -------------------------
        if self.entropy_penalty_coef > 0 and self.model.training \
                and hasattr(self.model, 'quantizer') \
                and hasattr(self.model.quantizer, '_last_soft_entropy'):
            soft_entropy = self.model.quantizer._last_soft_entropy
            max_entropy = torch.log(torch.tensor(
                float(self.model.codebook_size), device=device, dtype=torch.float32))
            # relu so we only penalise collapse, never reward surpassing max
            entropy_penalty = F.relu(max_entropy - soft_entropy)
            loss_dict["entropy_penalty"] = self.entropy_penalty_coef * entropy_penalty

        # -------------------------
        # MAE-style masked reconstruction (training only)
        # Random patches of the input are zeroed; the model must reconstruct
        # them from the remaining context, pushing codes to carry semantic content
        # beyond local texture.  The quantizer is put in eval-mode for this pass
        # so its EMA is not updated twice in the same step.
        # -------------------------
        if self.mae_mask_ratio > 0 and self.model.training \
                and obs.ndim == 4:
            B, C, H, W = obs.shape
            ps = self.mae_patch_size
            ph = (H + ps - 1) // ps   # patch grid height
            pw = (W + ps - 1) // ps   # patch grid width

            # Bernoulli mask over patch positions (1 = masked / held-out)
            patch_mask = (
                torch.rand(B, ph * pw, device=device) < self.mae_mask_ratio
            ).float()                                           # (B, ph*pw)

            # Expand from patch grid to pixel grid and crop to exact image size
            patch_mask_2d = patch_mask.view(B, 1, ph, pw)
            patch_mask_2d = patch_mask_2d.repeat_interleave(ps, dim=2) \
                                         .repeat_interleave(ps, dim=3)
            patch_mask_2d = patch_mask_2d[:, :, :H, :W]        # (B, 1, H, W)
            patch_mask_chan = patch_mask_2d.expand(-1, C, -1, -1)  # (B, C, H, W)

            masked_obs = obs * (1.0 - patch_mask_chan)  # zero masked patches

            # Forward pass; skip EMA update to avoid double-counting
            self.model.quantizer.eval()
            try:
                mae_recon, _, _, _ = self.model(masked_obs)
            finally:
                self.model.quantizer.train()

            if mae_recon.shape != obs.shape:
                mae_recon = F.interpolate(
                    mae_recon, size=obs.shape[-2:], mode='nearest')

            # Reconstruction loss on masked pixels only
            mae_sq = (obs - mae_recon).pow(2) * patch_mask_chan  # (B, C, H, W)
            n_masked = patch_mask_chan.sum().clamp_min(1.0)
            mae_loss = mae_sq.sum() / n_masked
            loss_dict["mae_loss"] = self.mae_loss_coef * mae_loss
        ...
        if return_stats:
            ...
            # Differentiable soft entropy (always available after quantizer forward)
            if hasattr(self.model, 'quantizer') \
                    and hasattr(self.model.quantizer, '_last_soft_entropy'):
                stats["soft_entropy"] = \
                    self.model.quantizer._last_soft_entropy.detach()
