...
class VectorQuantizerEMA(nn.Module):
    ...
    def forward(self, inputs, codebook_mask=None, masked_input=True, masked_output=True):
      ...
      # Differentiable soft entropy — gradient flows to encoder through distances.
      # Stored as an attribute so the trainer can add an entropy-penalty loss term.
      soft_probs_ent = F.softmax(-distances, dim=1)           # (B*N, K)
      avg_soft_probs_ent = soft_probs_ent.mean(dim=0)         # (K,)
      self._last_soft_entropy = -torch.sum(
          avg_soft_probs_ent * torch.log(avg_soft_probs_ent + 1e-10))
...
class VQVAEModel(nn.Module):
  def __init__(self, obs_dim, codebook_size, embedding_dim, encoder=None,
               ...
               # -------------------------
               # [NEW] Split-encoder: separate context branch with large RF
               # ctx_encoder runs in parallel with self.encoder (local branch).
               # ctx_cond_type: 'concat' (1x1 merge conv) or 'film' (scale/shift).
               # -------------------------
               ctx_encoder=None,
    ...
    # -------------------------
    # [NEW] Split-encoder context branch
    # -------------------------
    self.ctx_encoder = ctx_encoder
    self.ctx_cond_type = ctx_cond_type
    if ctx_encoder is not None:
      with torch.no_grad():
        ctx_out = ctx_encoder(test_input)
      ctx_c = ctx_out.shape[1]  # number of context channels
      if ctx_cond_type == 'film':
        self._film_scale = nn.Conv2d(ctx_c, self.embedding_dim, kernel_size=1)
        self._film_shift = nn.Conv2d(ctx_c, self.embedding_dim, kernel_size=1)
      else:  # 'concat'
        self._ctx_enc_merge = nn.Conv2d(
            self.embedding_dim + ctx_c, self.embedding_dim, kernel_size=1)
  ...
  def _get_decoder_input(self, quantized, encoder_out, x):
    """Compute decoder input, optionally conditioned on a context branch."""
    if self.ctx_encoder is not None:
      h_ctx = self.ctx_encoder(x)
      if self.ctx_cond_type == 'film':
        return quantized * self._film_scale(h_ctx) + self._film_shift(h_ctx)
      else:  # 'concat'
        return self._ctx_enc_merge(torch.cat([quantized, h_ctx], dim=1))
    elif self.use_context_cond:
      return self._ctx_merge(torch.cat([quantized, encoder_out], dim=1))
    return quantized

  def forward(self, x, code_dropout_rate=0.0, use_context=True):
    encoder_out = self.encoder(x)
    ...
    if self.sparsity_enabled and self.sparsity_mask is not None:
      ...
      dec_input = self._get_decoder_input(quantized, encoder_out, x) \
        if use_context else quantized
      ...
      # Random code dropout: zero out random latent positions so the decoder
      # cannot rely on any single code and must use contextual information.
      if code_dropout_rate > 0.0 and self.training:
        B = quantized.shape[0]
        spatial_shape = quantized.shape[2:]          # (H, W) or (N,)
        n_pos = int(np.prod(spatial_shape))
        keep_flat = torch.bernoulli(
            torch.full((B, n_pos), 1.0 - code_dropout_rate,
                       device=quantized.device))     # (B, n_pos)
        keep = keep_flat.view(B, 1, *spatial_shape)  # broadcast over C
  ...
  def encode(self, x, return_one_hot=False, as_long=True, as_grid=False,
    ...
    # Return flat continuous quantized embeddings (with straight-through grads)
    # This is the correct path for e2e RL: gradients flow through the
    # straight-through estimator in the quantizer back to the encoder.
    if return_quantized:
      return quantized.view(x.shape[0], -1)
