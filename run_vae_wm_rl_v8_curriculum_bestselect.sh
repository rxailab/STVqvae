...
RUN_DIR="${RUN_DIR:-$PROJECT_ROOT/wm_runs/vae_wm_rl_v8_curriculum_bestselect}"
...
echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] Starting v8_curriculum_bestselect"
...
exec "$PYTHON_BIN" -u full_train_eval.py \
  ...
  --n_train_unroll 12 \
  --rl_unroll_steps 16 \
  --rl_stage_unrolls 8,12,16 \
  --rl_stage_steps 100000,100000,100000 \
