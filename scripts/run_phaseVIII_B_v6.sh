#!/usr/bin/env bash
# Phase VIII-B: stop-gradient on PPO -> encoder. Encoder updated ONLY by the
# WM aux loss; PPO sees a detached encoder output. Tests whether joint
# encoder/WM training is the bimodality cause (Phase VIII-A confirmed both
# successful and failed encoders work offline → the joint optimization is
# the problem).
#
# Args: run_name, seed, eps
set -euo pipefail
REPO="/mmfs1/storage/users/xiar3/exp/STVqvae"
LOG_DIR="${REPO}/logs/phaseVIII_B"
mkdir -p "${LOG_DIR}"
...
if [[ -s "${SENTINEL}" ]]; then
  ...
  echo "[phaseVIII-B] ${RUN_NAME}  seed=${SEED}  eps=${EPS}  + stop-grad encoder"
  set +e
  python -u train.py \
    ...
    --sem_pre_vq \
    --explore_random_prob ${EPS} \
    --ppo_stop_grad_encoder > "${LOG}" 2>&1
  rc=$?; set -e
