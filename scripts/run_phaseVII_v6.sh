#!/usr/bin/env bash
# Phase VII: stabiliser interventions for the bimodal step-ε recipe.
# Args: run_name, seed, ε_value, intervention_flags
set -euo pipefail
REPO="/mmfs1/storage/users/xiar3/exp/STVqvae"
LOG_DIR="${REPO}/logs/phaseVII"
mkdir -p "${LOG_DIR}"
...
SEED="$2"
EPS="$3"
INTERVENTION_FLAGS="${4:-}"   # may include --explore_warmup_steps N, alternate --encoder_lr, or omit --encoder_lr_cosine
...
if [[ -s "${SENTINEL}" ]]; then
  ...
  echo "[phaseVII] ${RUN_NAME}  seed=${SEED}  eps=${EPS}  intervention=${INTERVENTION_FLAGS}"
  set +e
  python -u train.py \
