#!/usr/bin/env bash
# Phase K: horizon decomposition of imagined-vs-real rank correlation.
#
# Hypothesis: rank_corr is high at small K and degrades with K. If true, the
# practical fix to planning incoherence is "cap the planner horizon" — a
# concrete, paper-testable prescription. We sweep K∈{1,2,3,5,7,10} across all
# WM training conditions we have ckpts for (online / oracle / calib / multistep
# / valalign / conv) and aggregate.

set -euo pipefail
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CKPT_DIR="${REPO}/discrete_mbrl/model_free/models/MiniGrid-DoorKey-8x8-v0"
OUT="${REPO}/logs/phaseK"
mkdir -p "${OUT}"

# WM ckpts to evaluate. Tagged (label, file) pairs.
WMS=(
...
for entry in "${WMS[@]}"; do
  label="${entry%% *}"
  ckpt="${entry##* }"
  out="${OUT}/phaseK_${label}.json"
  log="${OUT}/phaseK_${label}.log"
