#!/usr/bin/env bash
# Phase N — action-conditioning probe across all WMs we've evaluated.
# Diagnoses Phase M's "WM is action-agnostic" finding by directly measuring
# how much T(z, a) varies with a.
set -euo pipefail
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CKPT_DIR="${REPO}/discrete_mbrl/model_free/models/MiniGrid-DoorKey-8x8-v0"
OUT="${REPO}/logs/phaseN"
mkdir -p "${OUT}"
WMS=(
...
for entry in "${WMS[@]}"; do
  ...
  out="${OUT}/phaseN_${label}.json"; log="${OUT}/phaseN_${label}.log"
