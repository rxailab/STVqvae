#!/usr/bin/env bash
# Phase V: seed replications of the Phase IV winner (step ε=0.50) plus
# Pareto fill-in points. Generic runner — takes run_name, seed, and
# extra flags.
set -euo pipefail
REPO="/mmfs1/storage/users/xiar3/exp/STVqvae"
LOG_DIR="${REPO}/logs/phaseV"
mkdir -p "${LOG_DIR}"
...
if [[ -s "${SENTINEL}" ]]; then
  ...
  echo "[phaseV] ${RUN_NAME}  seed=${SEED}  flags=${EXTRA_FLAGS}"
  set +e
  python -u train.py \
