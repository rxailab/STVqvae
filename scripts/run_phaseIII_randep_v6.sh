#!/usr/bin/env bash
# Phase III' (next paper §5 main result): random-EPISODE injection.
# At each env reset, with prob random_episode_prob mark the next episode
# as full-random. Diversifies the rollout STATE distribution at the
# trajectory level — addresses the state-coverage failure mode that
# step-level ε-greedy (Phase III) cannot fix.
#
# v6 DK-8 seed=1, random_episode_prob=0.5.
set -euo pipefail
...
if [[ -s "${MARKER}" ]]; then
  ...
  echo "[phaseIII-randep] ${RUN_NAME}  (random_episode_prob=0.5, seed=1)"
  set +e
  python -u train.py \
    ...
    --sem_pre_vq \
    --random_episode_prob 0.5 > "${LOG}" 2>&1
  rc=$?; set -e
